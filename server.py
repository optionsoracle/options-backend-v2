        ...
        for ticker in tickers:
            try:
                ...
                try:
                    ...
                    # earnings_dates gives a DataFrame with past and future earnings
                    try:
                        ed_df = stock.earnings_dates
                        if ed_df is not None and not ed_df.empty:
                            ed_df = ed_df.sort_index(ascending=False)
                            for idx in ed_df.index:
                                try:
                                    ...
                                    if dt < now and not last_earnings_date:
                                        last_earnings_date = dt.strftime("%Y-%m-%d")
                                    if dt >= now and not earnings_date:
                    ...
                    # Fallback: use get_earnings_dates with larger limit
                    if not last_earnings_date and not earnings_date:
                        try:
                            ed_df2 = stock.get_earnings_dates(limit=8)
                            if ed_df2 is not None and not ed_df2.empty:
                                ed_df2 = ed_df2.sort_index(ascending=False)
                                for idx in ed_df2.index:
                                    try:
                                        if hasattr(idx, "to_pydatetime"):
                                            dt = idx.to_pydatetime().replace(tzinfo=None)
                                        else:
                    ...
                    # If still no next earnings, estimate from last + 91 days
