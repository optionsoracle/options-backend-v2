...
def screen():
    try:
        ...
        strategy = body.get("strategy", "cc").lower()  # "cc" or "csp"
        ...
        for ticker in tickers:
            try:
                ...
                if strategy == "csp":
                    ...
                    if puts.empty:
                        results.append({"ticker": ticker, "error": "No puts available"})
                        continue

                    # Focus on strikes at or below current price (OTM/ATM puts)
                    puts = puts[puts["strike"] <= price * 1.02]
                    ...
                    for _, row in puts.iterrows():
                        ...
                        # CSP capital = cash reserved = (strike - premium) * 100
                        capital = (strike - premium) * 100
                        if capital <= 0:
                            continue

                        # Return on capital secured
                        trade_return = (premium * 100 / capital) * 100
                        ann_return = trade_return * (365 / dte)

                        # Break-even = effective purchase price if assigned
                        ...
                        in_the_money = strike > price  # put is ITM if strike > price

                        # Prob profit = probability stock stays ABOVE strike (put expires worthless)
                        prob_profit = None
                        try:
                            if iv and iv > 0 and dte > 0:
                                ...
                                prob_profit = round(n_d2 * 100, 1)  # N(d2) for puts
                        except Exception:
                            pass
                    ...
                    results.append({
                        ...
                        "earnings_warning": earnings_warning,
                        "iv_rank_label": iv_rank_label, "iv_rank_pct": iv_rank_pct,
                        "strikes": strike_results[:10],
                        "strategy": "csp"
                    })

                else:
                    # --- Covered Call logic (unchanged) ---
                    calls = chain.calls.copy()
