    ...
    try:
        ...
        for ticker in tickers:
            try:
                ...
                try:
                    # Try to get last earnings from quarterly financials
                    try:
                        fin = stock.quarterly_financials
                        if fin is not None and not fin.empty:
                            dates = sorted(fin.columns.tolist(), reverse=True)
                            if dates:
                                ld = dates[0]
                                if hasattr(ld, "date"):
                                    ld = ld.date()
                                last_earnings_date = str(ld)[:10]
                    except Exception:
                        pass

                    # Try quarterly income statement as fallback
                    ...
                    # Try to get next earnings from calendar
                    try:
                        cal = stock.calendar
                    ...
                    # If no next earnings from calendar, estimate from last + 90 days
