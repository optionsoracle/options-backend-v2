        ...
        for ticker in tickers:
            try:
                ...
                for _, row in calls.iterrows():
                    ...
                    # Uses Black-Scholes N(-d2) where iv is already in percent form
                    prob_profit = None
                    try:
                        if iv and iv > 0 and dte > 0:
                            iv_dec = iv / 100.0  # convert from percent to decimal
                            t = dte / 365.0
                            d2 = (math.log(price / strike) + (-0.5 * iv_dec**2) * t) / (iv_dec * math.sqrt(t))
                            # N(d2) = prob ITM, so 1 - N(d2) = prob OTM = prob of profit for call seller
