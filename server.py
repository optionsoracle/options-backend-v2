...
def get_earnings_dates(ticker):
    ...
    try:
        ...
        # Use yfinance's session - it already has cookies/crumb that Yahoo accepts
        stock = yf.Ticker(ticker)
        session = stock.session if hasattr(stock, 'session') and stock.session else None

        import requests as req_lib
        session = session or req_lib.Session()
        ...
        # First: try quoteSummary calendarEvents for next earnings
        try:
            url = f"https://query2.finance.yahoo.com/v10/finance/quoteSummary/{ticker}?modules=calendarEvents&corsDomain=finance.yahoo.com&formatted=false"
            resp = session.get(url, timeout=10)
            if resp.status_code == 200:
                cal = resp.json().get("quoteSummary", {}).get("result", [{}])[0].get("calendarEvents", {})
                ed_list = cal.get("earnings", {}).get("earningsDate", [])
                for ed in ed_list:
                    ...
                    if ts:
                        try:
                            ts = float(ts)
                            if ts > now_ts:
                                next_earnings_date = datetime.utcfromtimestamp(ts).strftime("%Y-%m-%d")
                                break
                        except Exception:
                            pass
        except Exception:
        ...
        # Second: try chart API for past earnings events
        try:
            url2 = f"https://query2.finance.yahoo.com/v8/finance/chart/{ticker}?interval=3mo&range=2y&events=earnings&corsDomain=finance.yahoo.com"
            resp2 = session.get(url2, timeout=10)
            if resp2.status_code == 200:
                events = resp2.json().get("chart", {}).get("result", [{}])[0].get("events", {})
                earnings_events = events.get("earnings", {})
                if earnings_events:
                    all_ts = []
                    for v in earnings_events.values():
                        ts = v.get("date")
                        if ts:
                            try:
                                ...
                                pass
                    past = sorted([t for t in all_ts if t < now_ts], reverse=True)
                    future = sorted([t for t in all_ts if t >= now_ts])
                    if past:
                        last_earnings_date = datetime.utcfromtimestamp(past[0]).strftime("%Y-%m-%d")
                    if future and not next_earnings_date:
                        next_earnings_date = datetime.utcfromtimestamp(future[0]).strftime("%Y-%m-%d")
        except Exception:
            pass

        # Fallback: yfinance calendar dict
        if not next_earnings_date or not last_earnings_date:
            try:
                cal = stock.calendar
                if isinstance(cal, dict):
                    ts = cal.get("Earnings Date")
                    if ts and isinstance(ts, list) and len(ts) > 0:
                        for ed in ts:
                            try:
                                ed_str = str(ed)[:10]
                                ed_dt = datetime.strptime(ed_str, "%Y-%m-%d")
                                if ed_dt.timestamp() > now_ts and not next_earnings_date:
                                    next_earnings_date = ed_str
                                elif ed_dt.timestamp() < now_ts and not last_earnings_date:
                                    last_earnings_date = ed_str
                            except Exception:
                                pass
            except Exception:
                pass
