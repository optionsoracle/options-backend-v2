...
def get_earnings_dates(ticker):
    ...
    try:
        ...
        try:
            ...
            to_date = (now + timedelta(days=400)).strftime("%Y-%m-%d")
...
def resolve_premium(bid, ask, last):
    """
    Resolve a real, tradeable premium from bid/ask/last.
    Rules:
      - If bid > 0 and ask > 0: use mid (bid+ask)/2
      - If bid > 0 only: use bid
      - If bid == 0 (no market on bid): use last price only if it looks reliable
        (last > 0 and ask > 0 and last <= ask). Never fabricate by halving ask.
      - Otherwise: return None (skip this strike — no reliable price)
    """
    ...
    if has_bid:
        return round(bid, 2)
    # bid is zero or missing — only use last if it's plausible vs ask
    if has_last and has_ask and last <= ask:
        return round(last, 2)
    if has_last and not has_ask:
        return round(last, 2)
    # No reliable price
    return None
...
def screen():
    try:
        ...
        view_mode = body.get("view_mode", "top_picks")
        # How many strikes to return per expiration group (frontend-controlled)
        strikes_per_group = int(body.get("strikes_per_group", 3))
        strikes_per_group = max(1, min(strikes_per_group, 20))  # clamp 1–20
        ...
        for ticker in tickers:
            try:
                ...
                for exp_dt, nearest_str in exps_to_scan:
                    ...
                    # Use the options chain's own underlying price where available.
                    # Use a local effective_price so we never mutate the outer price variable —
                    # a bad chain price on one expiration was previously corrupting all subsequent ones.
                    effective_price = price
                    try:
                        chain_price = safe_float(
                            chain.underlying.get('regularMarketPrice') or
                            chain.underlying.get('ask') or
                            chain.underlying.get('bid')
                        ) if hasattr(chain, 'underlying') and chain.underlying else None
                        if chain_price and chain_price > 0:
                            effective_price = chain_price
                    except Exception:
                        pass
                    ...
                    if strategy == "csp":
                        ...
                        if puts.empty:
                            continue
                        # OTM puts: from 70% of price up to just below current price
                        puts = puts[(puts["strike"] >= effective_price * 0.70) & (puts["strike"] <= effective_price * 1.001)]
                        for _, row in puts.iterrows():
                            ...
                            premium = resolve_premium(bid, ask, last)
                            ...
                            discount_pct = round((effective_price - breakeven) / effective_price * 100, 2)
                            discount_dollar = round(effective_price - breakeven, 2)
                            distance_pct = round((strike - effective_price) / effective_price * 100, 2)
                            in_the_money = strike > effective_price
                            prob_profit = None
                            try:
                                if iv and iv > 0 and dte > 0:
                                    t = dte / 365.0
                                    d2 = (math.log(effective_price / strike) + (-0.5 * iv**2) * t) / (iv * math.sqrt(t))
                        ...
                        if calls.empty:
                            continue
                        # Include strikes from slightly ITM up to 20% OTM
                        calls = calls[
                            (calls["strike"] >= effective_price * 0.98) &
                            (calls["strike"] <= effective_price * 1.20)
                        ]
                        for _, row in calls.iterrows():
                            ...
                            capital = (effective_price - premium) * 100
                            if capital <= 0:
                                continue
                            trade_return = (premium * 100 / capital) * 100
                            ann_return = trade_return * (365 / dte)
                            stock_gain = max(strike - effective_price, 0)
                            ...
                            if ann_return < min_ann or called_ann_return < min_called:
                                continue
                            breakeven = effective_price - premium
                            distance_pct = round((strike - effective_price) / effective_price * 100, 2)
                            in_the_money = strike < effective_price
                            prob_profit = None
                            try:
                ...
                if strategy == "csp":
                    ...
                    ranked = sorted(all_strike_results, key=lambda x: x["ann_return"], reverse=True)

                if view_mode == "spread_by_date":
                    from collections import defaultdict
                    by_exp = defaultdict(list)
                    for s in all_strike_results:
                        by_exp[s["expiration"]].append(s)
                    exp_groups = []
                    for exp_str, strikes in by_exp.items():
                        if strategy == "csp":
                            otm_g = sorted([s for s in strikes if not s["in_the_money"]], key=lambda x: x["ann_return"], reverse=True)
                            itm_g = sorted([s for s in strikes if s["in_the_money"]], key=lambda x: x["ann_return"], reverse=True)
                            sorted_strikes = otm_g + itm_g
                            best_visible = otm_g[0] if otm_g else (itm_g[0] if itm_g else None)
                        else:
                            sorted_strikes = sorted(strikes, key=lambda x: x["ann_return"], reverse=True)
                            best_visible = sorted_strikes[0] if sorted_strikes else None

                        # Use strikes_per_group from request (default 3, up to 20)
                        top_strikes = sorted_strikes[:strikes_per_group]
                        if not top_strikes or not best_visible:
                            continue

                        otm_top = [s for s in top_strikes if not s["in_the_money"]]
                        header_return = otm_top[0]["ann_return"] if otm_top else top_strikes[0]["ann_return"]
                        exp_groups.append({
                            "expiration": exp_str,
                            "dte": top_strikes[0]["dte"],
                            "best_ann_return": header_return,
                            "strikes": top_strikes
                        })
                    exp_groups.sort(key=lambda x: x["best_ann_return"], reverse=True)
                    exp_groups = exp_groups[:5]
                    best_exp = exp_groups[0]["expiration"]
                    best_dte = exp_groups[0]["dte"]
                    shown_exps = [datetime.strptime(g["expiration"], "%Y-%m-%d") for g in exp_groups]
                    earnings_warning = bool(earnings_dt and any(
                        0 <= (earnings_dt - exp).days <= 7 for exp in shown_exps
                    ))
                    results.append({
                        ...
                        "earnings_date": earnings_date, "earnings_warning": earnings_warning,
                        "iv_rank_label": iv_rank_label, "iv_rank_pct": iv_rank_pct,
                        "strikes": ranked[:strikes_per_group * 5],  # generous pool for cost-basis tool
                        "strikes_by_date": exp_groups,
                        "strategy": strategy
                    })
                else:
                    # top_picks / specific date
                    # Send a generous pool so the cost-basis tool has range to work with
                    pool_size = max(strikes_per_group * 5, 20)
                    best = ranked[:pool_size]
                    best_exp = best[0]["expiration"]
                    best_dte = best[0]["dte"]
                    unique_exps_top = list({datetime.strptime(s["expiration"], "%Y-%m-%d") for s in best})
                    earnings_warning = bool(earnings_dt and any(
                        0 <= (earnings_dt - exp).days <= 7 for exp in unique_exps_top
                    ))
                    results.append({
                        "ticker": ticker, "name": name,
                        "price": round(price, 2),
                        ...
                        "iv_rank_label": iv_rank_label, "iv_rank_pct": iv_rank_pct,
                        "strikes": best,
                        "all_strikes": ranked,  # full pool for cost-basis tool
                        "strategy": strategy
                    })
