        ...
        for ticker in tickers:
            try:
                ...
                if view_mode == "spread_by_date":
                    ...
                    earnings_warning = bool(earnings_dt and earnings_dt <= max(shown_exps))
                    results.append({
                    ...
                    all_for_cb = ranked
                    # Only warn if earnings fall on or before the latest expiration actually shown
                    shown_exps_top = [datetime.strptime(s["expiration"], "%Y-%m-%d") for s in best]
                    earnings_warning = bool(earnings_dt and earnings_dt <= max(shown_exps_top))
                    results.append({
                        "ticker": ticker, "name": name,
                        "price": round(price, 2),
                        "week_52_high": week_52_high, "week_52_low": week_52_low,
