...
@app.route("/debug_earnings/<ticker>")
def debug_earnings(ticker):
    ...
    try:
        ...
        if ed is not None and not ed.empty:
            result["earnings_dates"] = str(ed.head(8))
        else:
            result["earnings_dates"] = "empty or None"
    except Exception as e:
        result["earnings_dates_error"] = str(e)
    try:
        ed2 = stock.get_earnings_dates(limit=8)
        if ed2 is not None and not ed2.empty:
            result["get_earnings_dates"] = str(ed2.head(8))
        else:
            result["get_earnings_dates"] = "empty or None"
    except Exception as e:
        result["get_earnings_dates_error"] = str(e)
    try:
        cal = stock.calendar
...
def debug_earnings(ticker):
    try:
        ...
        try:
            cal = stock.calendar
            result["calendar_type"] = str(type(cal))
            if isinstance(cal, dict):
                result["calendar_keys"] = list(cal.keys())
                result["earningsTimestamp"] = cal.get("earningsTimestamp")
                result["earningsTimestampStart"] = cal.get("earningsTimestampStart")
                result["Earnings Date"] = str(cal.get("Earnings Date"))
            elif cal is not None:
                result["calendar_str"] = str(cal)[:500]
        except Exception as e:
            result["calendar_error"] = str(e)
        ...
        try:
            ...
            if inc is not None and not inc.empty:
                result["income_stmt_dates"] = [str(c)[:10] for c in inc.columns.tolist()[:4]]
        except Exception as e:
        ...
        for ticker in tickers:
            try:
                ...
                try:
                    ...
                    # Get NEXT earnings from calendar (most reliable source)
                    try:
                        cal = stock.calendar
                        if cal and isinstance(cal, dict):
                            # Try Unix timestamp first (most reliable)
                            ts = cal.get("earningsTimestamp") or cal.get("earningsTimestampStart")
                            if ts:
                                try:
                                    ed_dt = datetime.utcfromtimestamp(int(ts))
                                    if ed_dt > now:
                                    ...
                                    pass
                            # Fallback to string date in calendar
                    ...
                    # Get LAST earnings from income statement (accurate report dates)
                    try:
                        inc = stock.quarterly_income_stmt
                        if inc is not None and not inc.empty:
                            dates = sorted(inc.columns.tolist(), reverse=True)
                            for d in dates:
                                try:
                                    ...
                                    if dt < now:
                                        last_earnings_date = dt.strftime("%Y-%m-%d")
                                        break
                                except Exception:
                                    continue
                    except Exception:
                    ...
                    # Estimate next if still unknown
