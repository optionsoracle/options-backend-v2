    ...
    try:
        ...
        for ticker in tickers:
            try:
                ...
                latest_exp_dt = max(d for d, _ in exps_to_scan)
                earnings_debug = {
                    "earnings_dt": earnings_dt.strftime("%Y-%m-%d") if earnings_dt else None,
                    "latest_exp_dt": latest_exp_dt.strftime("%Y-%m-%d"),
                    "range_mode": range_mode,
                    "exp_date_to": exp_date_to if range_mode else None,
                }
                if earnings_dt:
                    if earnings_dt <= latest_exp_dt:
                ...
                if view_mode == "spread_by_date":
                    ...
                    best_dte = best[0]["dte"]
                    # Keep all_strikes sorted by ann_return (same as display order)
                    # so cost basis filtering preserves the best-return ordering
                    all_for_cb = ranked  # already sorted by ann_return desc
                    results.append({
                        "ticker": ticker, "name": name,
                        "price": round(price, 2),
                        "week_52_high": week_52_high, "week_52_low": week_52_low,
