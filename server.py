    ...
    try:
        ...
        for ticker in tickers:
            try:
                ...
                # After scanning all expirations: take the single best strike per
                # expiration, then sort across expirations by ann_return.
                # This gives true top-N across the range without any one expiration
                # monopolising the results with multiple ITM strikes.
                ...
                # Group by expiration, pick best per expiration, sort across expirations
                from collections import defaultdict
                by_exp = defaultdict(list)
                for s in all_strike_results:
                    by_exp[s["expiration"]].append(s)
                # Best strike per expiration
                best_per_exp = []
                for exp_strikes in by_exp.values():
                    exp_strikes.sort(key=lambda x: x["ann_return"], reverse=True)
                    best_per_exp.append(exp_strikes[0])
                best_per_exp.sort(key=lambda x: x["ann_return"], reverse=True)
                best = best_per_exp[:10]
