    ...
    try:
        ...
        for ticker in tickers:
            try:
                ...
                # Earnings dates — warning deferred until we know actual expirations scanned
                last_earnings_date, earnings_date = get_earnings_dates(ticker)
                earnings_warning = False  # will be re-evaluated after exps_to_scan is built
                earnings_dt = None
                if earnings_date:
                    try:
                ...
                # Now that we know actual expirations, evaluate earnings warning against the
                # latest expiration being shown — not the full scan range end date
                # earnings_warning evaluated after results are built (below)
                ...
                for exp_dt, nearest_str in exps_to_scan:
                    ...
                    # Use the options chain's own underlying price for distance calculations
                    # This avoids mismatch between stock.info price and options chain price
                    try:
                        ...
                        pass  # keep original price if underlying not available
                ...
                if view_mode == "spread_by_date":
                    ...
                    for exp_str, strikes in by_exp.items():
                        ...
                        if not top3 or not best_visible:
                            continue
                        # Use best OTM strike's return for the header so it matches what's displayed
                        # (hideITM on frontend filters out ITM strikes, so header must reflect OTM best)
                        otm_top3 = [s for s in top3 if not s["in_the_money"]]
                        header_return = otm_top3[0]["ann_return"] if otm_top3 else top3[0]["ann_return"]
                        exp_groups.append({
                            "expiration": exp_str,
                            "dte": top3[0]["dte"],
                    ...
                    # Only warn if earnings fall on or before the latest expiration actually shown
