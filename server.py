...
def get_earnings_dates(ticker):
    ...
    try:
        ...
        try:
            next_ts = yec.get_next_earnings_date(ticker.lower())
            if next_ts and int(next_ts) > now_ts:
                next_earnings_date = datetime.utcfromtimestamp(int(next_ts)).strftime("%Y-%m-%d")
        except Exception:
            pass

        try:
            all_earnings = yec.get_earnings_of(ticker.lower())
            if all_earnings:
                past = [e for e in all_earnings if e.get("startdatetime_timestamp", 0) < now_ts]
                if past:
                    past.sort(key=lambda e: e.get("startdatetime_timestamp", 0), reverse=True)
                    last_ts = past[0].get("startdatetime_timestamp")
                    if last_ts:
                        last_earnings_date = datetime.utcfromtimestamp(int(last_ts)).strftime("%Y-%m-%d")
        except Exception:
            pass
...
def debug_earnings(ticker):
    ...
    try:
        ...
        try:
            next_ts = yec.get_next_earnings_date(ticker.lower())
            result["next_ts"] = next_ts
        except Exception as e:
            result["next_ts_error"] = str(e)

        try:
            all_earnings = yec.get_earnings_of(ticker.lower())
            result["all_earnings_count"] = len(all_earnings) if all_earnings else 0
            result["all_earnings_sample"] = all_earnings[:2] if all_earnings else []
        except Exception as e:
            result["all_earnings_error"] = str(e)

    except Exception as e:
        result["package_loaded"] = False
