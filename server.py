...
def get_earnings_dates(ticker):
    ...
    try:
        ...
        # NASDAQ summary API - returns next earnings date in notifications
        try:
            url = f"https://api.nasdaq.com/api/quote/{ticker}/summary?assetClass=stocks"
            resp = req_lib.get(url, headers=headers, timeout=8)
            if resp.status_code == 200:
                data = resp.json().get("data", {}).get("summaryData", {})
                # Check NextEarningDate field
                ned = data.get("NextEarningDate", {})
                if isinstance(ned, dict):
                    val = ned.get("value", "")
                else:
                    val = str(ned) if ned else ""
                if val and val not in ("N/A", "", "None"):
                    try:
                        # Format is typically MM/DD/YYYY
                        if "/" in val:
                            ed_dt = datetime.strptime(val.strip(), "%m/%d/%Y")
                        else:
                            ed_dt = datetime.strptime(val.strip()[:10], "%Y-%m-%d")
                        if ed_dt > now:
                            next_earnings_date = ed_dt.strftime("%Y-%m-%d")
                    except Exception:
                        pass
        except Exception:
            pass

        # NASDAQ earnings history API - returns past earnings dates
        try:
            url2 = f"https://api.nasdaq.com/api/company/{ticker}/earnings?limit=4"
            resp2 = req_lib.get(url2, headers=headers, timeout=8)
            if resp2.status_code == 200:
                rows = resp2.json().get("data", {}).get("earningsList", {}).get("rows", [])
                for row in rows:
                    date_str = row.get("date", "")
                    if date_str and date_str not in ("N/A", ""):
                        try:
                            if "/" in date_str:
                                ed_dt = datetime.strptime(date_str.strip(), "%m/%d/%Y")
                            else:
                                ed_dt = datetime.strptime(date_str.strip()[:10], "%Y-%m-%d")
                            if ed_dt < now:
                                last_earnings_date = ed_dt.strftime("%Y-%m-%d")
                                break
                        except Exception:
                            pass
        except Exception:
...
def debug_earnings(ticker):
    ...
    try:
        ...
        if resp.status_code == 200:
            summary = resp.json().get("data", {}).get("summaryData", {})
            result["NextEarningDate"] = summary.get("NextEarningDate")
            result["all_keys"] = list(summary.keys())

        url2 = f"https://api.nasdaq.com/api/company/{ticker}/earnings?limit=4"
        resp2 = req_lib.get(url2, headers=headers, timeout=8)
        result["earnings_status_code"] = resp2.status_code
        if resp2.status_code == 200:
            rows = resp2.json().get("data", {}).get("earningsList", {}).get("rows", [])
            result["earnings_rows"] = rows[:2]
