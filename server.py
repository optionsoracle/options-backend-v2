    ...
    try:
        ...
        for ticker in tickers:
            try:
                ...
                # --- Earnings dates via Yahoo Finance scraper ---
                earnings_date = None
                ...
                try:
                    ...
                    url = f"https://query1.finance.yahoo.com/v8/finance/chart/{ticker}?interval=3mo&range=1y"
                    resp = requests.get(url, headers=headers, timeout=8)
                    if resp.status_code == 200:
                        data_yf = resp.json()
                        events = data_yf.get("chart", {}).get("result", [{}])[0].get("events", {})
                        earnings_events = events.get("earnings", {})
                        if earnings_events:
                            all_dates = []
                            for k, v in earnings_events.items():
                                ts = v.get("date") or v.get("startdatetime")
                                if ts:
                                    try:
                            ...
                            if all_dates:
                                now = datetime.utcnow()
                                past = sorted([d for d in all_dates if d < now], reverse=True)
                                future = sorted([d for d in all_dates if d >= now])
                                if past:
                                    last_earnings_date = past[0].strftime("%Y-%m-%d")
                                if future:
                                    earnings_date = future[0].strftime("%Y-%m-%d")
                                    if datetime.strptime(earnings_date, "%Y-%m-%d") <= req_date:
                                        earnings_warning = True

                    # If no next earnings found, estimate from last + 91 days
